...
const PORT_NAME = 'tydbuddy-keepalive'
  ...
  try {
    await chrome.scripting.executeScript({
      ...
      func: (portName, interval) => {
        if (window._tydbuddyKeepAlive) return
        ...
        function connect() {
          try {
            ...
            window._tydbuddyKeepAlive = { port, timer }
...
export async function stopKeepAlive(tabId) {
  ...
  try {
    await chrome.scripting.executeScript({
      target: { tabId },
      func: () => {
        if (window._tydbuddyKeepAlive) {
          clearInterval(window._tydbuddyKeepAlive.timer)
          try { window._tydbuddyKeepAlive.port.disconnect() } catch { /* noop */ }
          window._tydbuddyKeepAlive = null
        }
      },
    })
