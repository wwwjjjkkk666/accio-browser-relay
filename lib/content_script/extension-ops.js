  ...
  const results = await chrome.scripting.executeScript({
    ...
    func: (options) => {
      ...
      for (const el of candidates) {
        ...
        el.setAttribute('data-tydbuddy-idx', String(idx))
  ...
  const results = await chrome.scripting.executeScript({
    ...
    func: (p) => {
      ...
      if (p.index != null) {
        ...
        el = deepQuery(document, `[data-tydbuddy-idx="${idx}"]`)
      } else if (p.selector) {
...
async function showClickRipple(tabId, x, y) {
  await chrome.scripting.executeScript({
    target: { tabId },
    func: (cx, cy) => {
      if (!document.getElementById('tydbuddy-ripple-style')) {
        const s = document.createElement('style')
        s.id = 'tydbuddy-ripple-style'
        s.textContent = '@keyframes tydbuddy-r{0%{transform:scale(0);opacity:1}100%{transform:scale(2.5);opacity:0}}'
      ...
      Object.assign(d.style, {
        ...
        background: 'rgba(43, 156, 231, 0.4)', pointerEvents: 'none',
        zIndex: '2147483647', animation: 'tydbuddy-r .8s ease-out forwards',
      })
