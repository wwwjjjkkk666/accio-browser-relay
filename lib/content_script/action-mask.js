...
export async function extActionMask(tabId, params) {
  ...
  const results = await chrome.scripting.executeScript({
    target: { tabId },
    func: (maskState) => {
      const MASK_ID = 'tydbuddy-action-mask'
      ...
      if (!host) {
        ...
        style.textContent = `
          ...
          .glow-border {
            ...
            pointer-events: none;
            box-shadow: inset 0 0 30px rgba(43, 156, 231, 0.15);
            animation: tydbuddy-glow 1.5s ease-in-out infinite alternate;
          }
          @keyframes tydbuddy-glow {
            from { opacity: 0.7; }
            to { opacity: 1; }
          }
          .status-bar {
          ...
          .status-dot {
            ...
            background: #2b9ce7;
            animation: tydbuddy-pulse 1s ease-in-out infinite alternate;
          }
          @keyframes tydbuddy-pulse {
            from { opacity: 0.5; }
            to { opacity: 1; }
          }
        `
        ...
        bar.innerHTML = '<span class="status-dot"></span><span>TydBuddy is working…</span>'
