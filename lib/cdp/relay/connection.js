...
function setState(newState) {
  ...
  void chrome.storage.local.set({ tydbuddy_relayState: newState })
}
...
//
// 守卫策略（防 reload 循环）：
//   chrome.storage.local.tydbuddy_relayReloadState = { targetKey, at }
...
const RELOAD_STORAGE_KEY = 'tydbuddy_relayReloadState'
...
function notifyError(title, message) {
  chrome.notifications.create('tydbuddy-relay-error', {
    type: 'basic',
    iconUrl: 'icons/icon128.png',
    title: `TydBuddy Relay: ${title}`,
    message,
    priority: 2,
  }, () => {
    if (chrome.runtime.lastError) {
      console.warn('[tydbuddy-relay] notification create failed:', chrome.runtime.lastError.message)
    }
  })
}
...
async function getControlPort() {
  ...
  const raw = Number.parseInt(String(stored.tydbuddy_controlPort || ''), 10)
  if (Number.isFinite(raw) && raw > 0 && raw <= 65535) return raw
  const relayRaw = Number.parseInt(String(stored.tydbuddy_relayPort || ''), 10)
  if (Number.isFinite(relayRaw) && relayRaw > 0 && relayRaw <= 65535) {
    const inferred = clampPort(relayRaw - RELAY_PORT_OFFSET)
    void chrome.storage.local.set({ tydbuddy_controlPort: inferred })
    return inferred
  }
...
export async function setRelayEnabled(enabled) {
  await chrome.storage.local.set({ tydbuddy_relayEnabled: enabled })
}

export async function isRelayEnabled() {
  const stored = await chrome.storage.local.get(['tydbuddy_relayEnabled'])
  return stored.tydbuddy_relayEnabled === true
}
...
export function trySendToRelay(payload) {
  try {
    ...
    console.debug('[tydbuddy-relay] trySendToRelay failed:', err)
  }
}
...
async function onRelayMessage(rawText) {
  ...
  if (msg?.method === 'Extension.helloAck') {
    ...
    if (p.status === 'ok') {
      if (p.encrypted === true) {
        activateEncryption()
        void chrome.storage.local.remove('_tydbuddy_relayError')
        ...
        void chrome.storage.local.set({
          _tydbuddy_relayError: 'encryption_unsupported',
        })
        if (_handshakeReject) {
...
export async function toggle() {
  ...
  try {
    ...
    console.warn('[tydbuddy-relay] initial connect failed:', message)
