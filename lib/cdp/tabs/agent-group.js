 ...
 *
 * Creates and manages a Chrome tab group labeled "TydBuddy Agent"
 ...
 * "TydBuddy Agent" group rather than creating a duplicate.
 *
 * dissolve() clears every tab group whose title matches TydBuddy Agent (not only one),
...
export class AgentGroupManager {
   ...
   *
   * Finds every matching "TydBuddy Agent" tab group (plus stale cached #groupId if valid).
  ...
  async dissolve(agentTabIds) {
    ...
    if (uniqueGroupIds.length === 0) {
      log.info('dissolve: no TydBuddy Agent tab group found, skipping')
      return
    }
  ...
  /**
   * Close every tab in every Chrome tab group whose title matches TydBuddy Agent.
  ...
  async closeAllTydBuddyAgentGroupTabs() {
    ...
    try {
      ...
      log.warn('closeAllTydBuddyAgentGroupTabs: tabGroups query failed:', err)
    }
    const uniqueGroupIds = [...new Set(groupIds)]
    let closed = 0
    for (const gid of uniqueGroupIds) {
      try {
        ...
        log.warn('closeAllTydBuddyAgentGroupTabs: failed for group', gid, err)
      }
    }
    log.info('closeAllTydBuddyAgentGroupTabs: groups=', uniqueGroupIds.length, 'tabsClosed=', closed)
