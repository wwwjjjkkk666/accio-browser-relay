...
export function createTargetOps(mgr) {
  ...
  async function cdpCreateTarget(params) {
    ...
    return { targetId: attached.vtabId, retained: retain }
  }
  ...
  async function cdpActivateTarget(params, fallbackTabId) {
    ...
    if (!tab) {
      // Tab no longer exists — notify relay and clean up local state
      mgr.removeTrackedEntry(toActivate, 'tab-closed')
      mgr.deleteAgent(toActivate)
      throw new Error(`Tab ${toActivate} no longer exists for Target.activateTarget`)
    }
    if (tab.windowId) {
