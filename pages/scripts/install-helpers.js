...
function setupExtensionPath() {
  ...
  if (el) {
    ...
    var safeFolderName = escapeHtml(folderName);
    var shortHtml =
      '<span class="path-short">' +
      escapeHtml(shortPath) +
      '<span class="path-highlight">' +
      safeFolderName +
      '</span></span>';

    var highlightStart = folderName && extensionPath.endsWith(folderName)
      ? extensionPath.length - folderName.length
      : -1;
    var fullHtml;
    if (highlightStart >= 0) {
      fullHtml =
        '<span class="path-full">' +
        escapeHtml(extensionPath.slice(0, highlightStart)) +
        '<span class="path-highlight">' +
        safeFolderName +
        '</span></span>';
    } else {
      fullHtml = '<span class="path-full">' + escapeHtml(extensionPath) + '</span>';
    }

    el.innerHTML = shortHtml + fullHtml;
    el.title = escapeHtml(extensionPath);
  }
